...
export default {
    ...
    setup() {
        ...
        async function fetchMovies() {
            await fetch(
                `https://api.themoviedb.org/3/search/movie?api_key=${import.meta.env.VITE_API_KEY}&include_adult=false&query=${route.query.q}`
            )
        ...
        async function appendInfo(value) {
            await fetch(
                `https://api.themoviedb.org/3/movie/${value}?api_key=${import.meta.env.VITE_API_KEY}&append_to_response=external_ids,videos,credits,release_dates,similar`
            )
                .then((response) => response.json())
                .then((response) => {
